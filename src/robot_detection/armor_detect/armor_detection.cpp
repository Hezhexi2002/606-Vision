...
namespace robot_detection {
    ArmorDetection::ArmorDetection() { //构造函数以及装甲板参数初始化
        last_armor_center_ = cv::Point2f(0, 0);
        armor_height = 60;
        armor_width = 127.5;
        SolveArmorCoordinate(armor_width, armor_height); //pnp姿态解算
    }
    /**
     * @brief 加载各项参数
     * 
     * @param color 敌方颜色
     * @param enable_debug 是否开启debug
     * @param light_min_angle 
     * @param light_max_angle_diff 
     * @param light_aspect_ratio 
     * @param armor_max_angle 
     * @param armor_max_aspect_ratio 
     * @param armor_min_aspect_ratio 
     * @param blue_threshold 蓝色阈值
     * @param red_threshold 红色阈值
     * @param red_brightness_threshold 红色亮度阈值 
     * @param blue_brightness_threshold 蓝色亮度阈值
     * @param intrinsic_matrix 相机内参矩阵
     * @param distortion_coeffs 相机畸变系数
     */
     ...
     * @brief 装甲板检测函数
     * 
     * @param src_img_ 待检测的图像
     * @param detected 是否检测到装甲板 bool类型
     * @param target_3d 目标装甲板的坐标，其中z代表distance
     * @param target_r 目标装甲板的旋转角度
     */
    void ArmorDetection::DetectArmor(cv::Mat &src_img_, bool &detected, cv::Point3f &target_3d, cv::Point3f &target_r) {
        ...
        if (!armors.empty()) {
            ...
            if (enable_debug_) { //开启debug模式下可视化检测框
                cv::Point2f vertex[4];
     ...
     * @brief 灯条检测函数
     * 
     * @param src_img_ 待检测图像
     * @param lights 候选灯条
     * @param lights_angle 灯条角度
     */
    void ArmorDetection::DetectLights(const cv::Mat &src_img_, std::vector<cv::RotatedRect> &lights, std::vector<double> &lights_angle) {
        ...
        if (enemy_color_ == false)
        {
            ...
            cv::threshold(gray_img_, binary_brightness_img_, blue_brightness_threshold_, 255, cv::THRESH_BINARY);
        }
        else
        {
            thresh = red_threshold_;
            cv::threshold(gray_img_, binary_brightness_img_, red_brightness_threshold_, 255, cv::THRESH_BINARY);
        }
        cv::threshold(light, binary_color_img_, thresh, 255, cv::THRESH_BINARY);
        ...
        cv::findContours(binary_color_img_, contours_brightness, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
     ...
     * 
     * @param lights 候选灯条
     * @param armors_possible 符合条件的装甲板
     * @param lights_angle 灯条角度
     */
     ...
     * @brief 筛选最终的装甲板
     * 
     * @param armors 符合条件的装甲板
     * @param target_3d_ 装甲板坐标
     * @return 筛选得到装甲板
     */
     ...
     * 
     * @param src_img_ 同上
     * @param armor 最终筛选出来的装甲板
     * @param target_3d 目标装甲板的三维坐标，实际上就是解算出来的tvec
     * @param target_r 目标装甲板的旋转角度，实际上就是解算出来的rvec
     */
